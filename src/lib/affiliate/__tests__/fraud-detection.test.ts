...
function hasHighConversionRate(
  converted: number,
  total: number,
  threshold: number = 80
): boolean {
...
describe('Fraud Detection - Property-Based Tests', () => {
  ...
  describe('High Conversion Rate Detection', () => {
    it('should flag conversion rates above threshold', () => {
      ...
      fc.assert(
        fc.property(fc.integer({ min: 11, max: 100 }), (total) => {
  ...
  describe('Rapid Conversions Detection', () => {
    it('should flag conversions within 24 hours', () => {
      ...
      const conversions = Array.from(
        { length: 5 },
        (_, i) => new Date(baseDate.getTime() + i * 2 * 60 * 60 * 1000) // 2 hours apart
      );
    ...
    it('should not flag conversions spread over time', () => {
      ...
      const conversions = Array.from(
        { length: 5 },
        (_, i) => new Date(baseDate.getTime() + i * 10 * 60 * 60 * 1000) // 10 hours apart
      );
    ...
    it('should not flag small numbers of conversions', () => {
      ...
      fc.assert(
        fc.property(fc.integer({ min: 0, max: 4 }), (count) => {
          ...
          const conversions = Array.from(
            { length: count },
            (_, i) => new Date(baseDate.getTime() + i * 1000)
          );
          const result = hasRapidConversions(conversions);
          expect(result).toBe(false);
        }),
